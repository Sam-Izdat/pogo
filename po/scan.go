        ...
        for fn, f := range pkg.Files {
            ast.Inspect(f, func(n ast.Node) bool {
                ...
                case *ast.CallExpr:
                    ...
                    if len(msgs) > 0 { res = append(res, msgs...) }
                }
                return true
            })
        }
...
func ScanTmpl(path string) (res []spec.Msg) {
    filepath.Walk(path, func(fp string, fi os.FileInfo, err error) error {
        ...
        if matched {
            ...
            if err != nil { panic(err) }
            tmpl := string(buf)
            res = append(res, scanTmplString(tmpl)...)
            for k := range res {
                res[k].Filename = fp
            }
        }
...
func prepMsg(msgs *[]spec.Msg) {
    ...
    for k, v := range *msgs {
        ...
        // remove quotes
        if len(v.Ctxt) > 0 { (*msgs)[k].Ctxt = v.Ctxt[1:len(v.Ctxt)-1] }
        if len(v.Id) > 0 { (*msgs)[k].Id = v.Id[1:len(v.Id)-1] }
        if len(v.IdPlural) > 0 { (*msgs)[k].IdPlural = v.IdPlural[1:len(v.IdPlural)-1] }
    }
}
